        ...
        @if (!supabase.isLoggedIn()) {
          ...
          <div class="portal-dashboard opacity-0 translate-y-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4">
              <div>
                <h2 class="text-4xl font-display font-medium mb-2">{{ 'PORTAL.WELCOME_BACK' | translate }}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div class="glass-panel p-8 rounded-[2rem] border border-[var(--accent-main)] hover:shadow-[0_0_40px_rgba(var(--accent-main-rgb),0.2)] transition-all group relative overflow-hidden">
                ...
                <h3 class="text-xl font-display font-medium mb-3">{{ 'PORTAL.FORGE_TITLE' | translate }}</h3>
                <p class="text-sm text-[var(--text-muted)]">{{ 'PORTAL.FORGE_DESC' | translate }}</p>
                ...
                <div class="mt-6 pt-6 border-t border-[var(--text-primary)]/10 flex justify-between items-center">
                  @if (supabase.hasForgeAccess()) {
                    ...
                    <span class="text-[10px] uppercase tracking-widest text-green-400 border border-green-400/30 px-2 py-1 rounded-full bg-green-400/10">{{ 'PORTAL.AUTHORIZED' | translate }}</span>
                  } @else {
                    <div class="flex flex-col gap-4 w-full">
                      <div class="flex justify-between items-center">
                        <span class="text-lg font-display font-medium text-[var(--text-primary)]">$9.99 <span class="text-[10px] text-[var(--text-muted)] uppercase tracking-tighter">{{ 'PORTAL.ONE_TIME' | translate }}</span></span>
                        <span class="text-[10px] uppercase tracking-widest text-[var(--accent-main)] border border-[var(--accent-main)]/30 px-2 py-1 rounded-full bg-[var(--accent-main)]/10">{{ 'PORTAL.ACCESS_REQUIRED' | translate }}</span>
                      </div>
                      <div class="flex gap-2">
                        <button (click)="router.navigate(['/contact'])" class="flex-1 text-[10px] py-2 rounded-lg bg-[var(--text-primary)] text-[var(--bg-main)] font-bold uppercase tracking-widest hover:scale-105 transition-transform">{{ 'PORTAL.BUY_ACCESS' | translate }}</button>
                      </div>
                    </div>
                  }
                </div>
              ...
              <div class="glass-panel p-8 rounded-[2rem] border border-[var(--text-primary)]/10 hover:border-[var(--accent-main)]/50 transition-colors group">
                ...
                <h3 class="text-xl font-display font-medium mb-3">{{ 'PORTAL.PROJECT_TRACKING' | translate }}</h3>
                ...
                <div class="mt-6 pt-6 border-t border-[var(--text-primary)]/10">
                  <span class="text-xs font-mono text-[var(--accent-main)] uppercase tracking-widest">{{ 'PORTAL.ACTIVE_PROJECTS' | translate }}</span>
                </div>
              </div>
              ...
              <div class="glass-panel p-8 rounded-[2rem] border border-[var(--text-primary)]/10 hover:border-[var(--accent-main)]/50 transition-colors group">
                ...
                <h3 class="text-xl font-display font-medium mb-3">{{ 'PORTAL.PRIORITY_SUPPORT' | translate }}</h3>
                <p class="text-sm text-[var(--text-muted)]">{{ 'PORTAL.PRIORITY_SUPPORT_DESC' | translate }}</p>
                <div class="mt-6 pt-6 border-t border-[var(--text-primary)]/10">
                  <button class="text-xs font-mono text-[var(--text-primary)] hover:text-[var(--accent-main)] uppercase tracking-widest transition-colors flex items-center gap-1">
                    {{ 'PORTAL.OPEN_TICKET' | translate }} <span class="material-icons text-[14px]">arrow_forward</span>
                  </button>
                </div>
              </div>
            </div>
...
export class PortalComponent {
  ...
  constructor() {
    afterNextRender(async () => {
      ...
      // Password-recovery links sometimes land here — send user to set-password UI
